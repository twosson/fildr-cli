...
func newDRBDCollector(logger log.Logger) (gateway.Collector, error) {
...
func (c *drbdCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for scanner.Scan() {
		...
		if len(kv) != 2 {
			c.logger.Debugf("msg", "skipping invalid key:value pair", "field", field)
			continue
		}
		...
		c.logger.Debugf("msg", "unhandled key-value pair", "key", kv[0], "value", kv[1])
	}
