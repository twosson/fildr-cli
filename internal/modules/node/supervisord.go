...
func NewSupervisordCollector(logger log.Logger) (gateway.Collector, error) {
	var (
...
func (c *supervisordCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, p := range res.(xmlrpc.Array) {
		...
		c.logger.Debugf("msg", "process info", "group", info.Group, "name", info.Name, "state", info.StateName, "pid", info.PID)
	}
