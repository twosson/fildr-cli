...
func NewTextFileCollector(logger log.Logger) (gateway.Collector, error) {
	c := &textFileCollector{
...
func convertMetricFamily(metricFamily *dto.MetricFamily, ch chan<- prometheus.Metric, logger log.Logger) {
	...
	for _, metric := range metricFamily.Metric {
		if metric.TimestampMs != nil {
			logger.Warnf("msg", "Ignoring unsupported custom timestamp on textfile collector metric", "metric", metric)
		}
...
func (c *textFileCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil && c.path != "" {
		errored = true
		c.logger.Errorf("msg", "failed to read textfile collector directory", "path", c.path, "err", err)
	}
	...
	for _, f := range files {
		...
		if err != nil {
			errored = true
			c.logger.Errorf("msg", "failed to collect textfile data", "file", f.Name(), "err", err)
			continue
		}
