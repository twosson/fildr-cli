...
func NewWifiCollector(logger log.Logger) (gateway.Collector, error) {
	const (
		subsystem = "wifi"
	)
...
func (c *wifiCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, ifi := range ifis {
		...
		c.logger.Debugf("msg", "probing wifi device with type", "wifi", ifi.Name, "type", ifi.Type)
		...
		case errors.Is(err, os.ErrNotExist):
			c.logger.Debugf("msg", "BSS information not found for wifi device", "name", ifi.Name)
		default:
		...
		case errors.Is(err, os.ErrNotExist):
			c.logger.Debugf("msg", "station information not found for wifi device", "name", ifi.Name)
		default:
